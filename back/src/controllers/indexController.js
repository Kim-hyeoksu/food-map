...
exports.readRestaurants = async function (req, res) {
  ...
  try {
    const connection = await pool.getConnection(async (conn) => conn);  //pool: mysql에 접근할 수 있는 객체
    try {
      const [rows] = await indexDao.selectRestaurants(connection, category); //mysql 접속 정의
      //비구조 할당
      return res.send({
        result: rows,
        isSuccess: true,
...
exports.deleteStudent = async function (req, res) {
  ...
  try {
    ...
    try {
      ...
      const [rows] = await indexDao.deleteStudents(connection, studentIdx); //mysql 접속 정의
      //비구조 할당
      return res.send({
        isSuccess: true,
        code: 200, // 요청 실패시 400번대 코드
...
exports.example = async function (req, res) {
  try {
    const connection = await pool.getConnection(async (conn) => conn);  //pool: mysql에 접근할 수 있는 객체
    try {
      const [rows] = await indexDao.exampleDao(connection); //mysql 접속 정의
      //비구조 할당
      return res.send({
        result: rows,
        isSuccess: true,
...
exports.updateStudent = async function(req, res) {
  ...
  try {
    ...
    try {
      ...
      const [rows] = await indexDao.updateStudents(connection, studentIdx, studentName, major, birth, address); //mysql 접속 정의
      //비구조 할당
      return res.send({
        isSuccess: true,
        code: 200, // 요청 실패시 400번대 코드
...
exports.createStudent = async function(req, res) {
  ...
  try {
    const connection = await pool.getConnection(async (conn) => conn);  //pool: mysql에 접근할 수 있는 객체
    try {
      const [rows] = await indexDao.insertStudents(connection, studentName, major, birth, address); //mysql 접속 정의
      //비구조 할당
      return res.send({
        isSuccess: true,
        code: 200, // 요청 실패시 400번대 코드
...
exports.readStudents = async function(req, res) {
  ...
  try {
    const connection = await pool.getConnection(async (conn) => conn);  //pool: mysql에 접근할 수 있는 객체
    try {
      const [rows] = await indexDao.selectStudents(connection, studentIdx); //mysql 접속 정의
      //비구조 할당
      return res.send({
        result: rows,
        isSuccess: true,
